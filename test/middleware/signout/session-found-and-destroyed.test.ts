...
test('signOutMiddleware should destroy session when found in session store', async t => {
    ...
    // const stubSession = stubInterface<Session>()
    // stubSession.sessionId = testSessionId
    ...
    t.true(ctx.done)
    t.true(storeStub.getByPair.calledOnce)
